...
#
# OMV-compatible version of SMR detection script
# This version uses standard Linux commands instead of TrueNAS-specific midclt
# Based on the original script from Basil Hendroff Github repository
#
...
if [[ "$1" == "--test" ]]; then
    TEST_MODE=1
    echo "Running in TEST MODE - will show test SMR drive detection"
    echo
fi

# Function to gather drive information for OMV systems
gather_drive_info() {
    # Use lsblk and smartctl to gather drive information
    declare -ag Device=()
    declare -ag Model=()
    declare -ag Serial=()
    
    # Get list of block devices (excluding partitions, loops, etc.)
    local drives=$(lsblk -d -n -o NAME,TYPE | grep "disk" | awk '{print $1}')
    
    local i=0
    for drive in $drives; do
        # Skip if not a physical drive
        if [[ "$drive" =~ ^(loop|sr|dm-) ]]; then
            continue
        fi
        
        # Get model and serial from smartctl
        local smart_info
        if smart_info=$(smartctl -i /dev/$drive 2>/dev/null); then
            local model=$(echo "$smart_info" | grep "Device Model:" | sed 's/Device Model:[[:space:]]*//' | xargs)
            local serial=$(echo "$smart_info" | grep "Serial Number:" | sed 's/Serial Number:[[:space:]]*//' | xargs)
            
            # Fallback to Product and Vendor for some drives
            if [[ -z "$model" ]]; then
                local vendor=$(echo "$smart_info" | grep "Vendor:" | sed 's/Vendor:[[:space:]]*//' | xargs)
                local product=$(echo "$smart_info" | grep "Product:" | sed 's/Product:[[:space:]]*//' | xargs)
                if [[ -n "$vendor" && -n "$product" ]]; then
                    model="$vendor $product"
                fi
            fi
            
            # Only add if we have valid model and serial
...
# Test mode - simulate SMR detection with a real drive from your system
if [[ "$TEST_MODE" == 1 ]]; then
    # Get first available drive for testing
    if [[ ${#Device[@]} -gt 0 ]]; then
        echo "Test: Simulating SMR detection with drive ${Device[0]} (${Model[0]})"
        echo "Creating temporary test array with current drive model..."
        # Create a test array that will match the first drive
        model_prefix=$(echo "${Model[0]}" | awk '{print $1, $2}')
        model_suffix=$(echo "${Model[0]}" | awk '{print $NF}')
        TEST=("$model_prefix" "TEST" "$model_suffix")
        echo "TEST array: ${TEST[@]}"
        echo
    fi
fi

# Gather drive information for OMV
gather_drive_info

echo "Detected drives for SMR analysis:"
for ((i=0;i<${#Device[@]};i++)); do
    echo "  /dev/${Device[$i]}: ${Model[$i]} (S/N: ${Serial[$i]})"
done
echo
...
if [[ "$f" == 0 ]]; then
  echo
  if [[ "$TEST_MODE" == 1 ]]; then
    echo -e "\e[1;33mTEST MODE: No SMR drives detected in normal operation.\e[0m"
    echo -e "\e[1;33mThis confirms the SMR detection system is working properly.\e[0m"
  else
...
# otherwise, one or more SMR drives were detected so display all SMR drives in a table :(
  echo
  if [[ "$TEST_MODE" == 1 ]]; then
    echo -e "\e[1;33mTEST MODE: SMR detection triggered (this is expected in test mode).\e[0m"
  else
